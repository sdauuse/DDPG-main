...
public class V4Test {
    ...
    /**
     * sse-V4：function调用
     * 1.创建一个空的消息列表 messages，并向其中添加一个用户发送的消息。
     * 2.构建一个 ChatCompletionRequest 对象，该对象包含了调用模型API所需的参数，如模型名称、消息列表、请求ID等。
     * 3.调用模型API，并将结果保存在 sseModelApiResp 对象中。
     * 4.如果调用成功，则处理返回的数据：
     * 5.创建一个 ChatMessageAccumulator 对象来处理模型API返回的响应流。
     * 6.从响应流中提取工具调用和内容，并输出到控制台。
     * 7.构建一个 Choice 对象和一个 ModelData 对象，其中包含了对模型API返回数据的解析结果。
     * 8.最后，输出模型API的返回结果 sseModelApiResp，并转换为JSON格式输出。
     */
    @Test
    public void testFunctionSSE() {
        List<ChatMessage> messages = new ArrayList<>();
        ChatMessage chatMessage = new ChatMessage(ChatMessageRole.USER.value(), "假设你是我的虚拟女朋友，请你以女朋友的口吻回答我以下问题：今天吃什么好呢？");
    ...
    public void testFunctionInvoke(){
        ...
        try {
            ...
            System.out.println("data:" + invokeModelApiResp.getData().getChoices().get(0).getMessage().getContent());
